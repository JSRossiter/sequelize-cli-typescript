...
const factory = (sequelize: Sequelize, DataTypes: typeof DataTypesType) => {
  class <%= name %>
    ...
    <% attributes.forEach(function(attribute, index) { %>
      public readonly <%= attribute.fieldName %>: <%= attribute.dataFunction ? `${attribute.dataFunction.toUpperCase()}(DataTypes.${attribute.dataType.toUpperCase()})` : attribute.dataValues ? attribute.dataValues.split(', ').join(' | ') : attribute.jsType %>;
    <% }) %>
