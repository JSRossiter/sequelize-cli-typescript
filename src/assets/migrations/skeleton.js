...
module.exports = {
  up: async (queryInterface, Sequelize) => {<% attributes.forEach(function(attribute) { %>
  ...
  down: async (queryInterface, Sequelize) => {
    <% attributes.forEach(function(attribute) { %>await queryInterface.removeColumn('<%= tableName %>', '<%= attribute.fieldName %>')
    <% }) %>
  }
};
